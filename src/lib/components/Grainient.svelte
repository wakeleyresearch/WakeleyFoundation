<!--
  Adapted from Svelte Bits Grainient by David Haz.
  Source: https://github.com/DavidHDev/svelte-bits
...
void main(){ vec4 o=vec4(0.0); mainImage(o,gl_FragCoord.xy); fragColor=o; }`;
    ...
    const loop = (time: number) => {
      (program.uniforms.iTime as { value: number }).value = (time - startedAt) * 0.001;
      ...
      const centerOffset = (program.uniforms.uCenterOffset as { value: Float32Array }).value;
      centerOffset[0] = current.centerX;
      centerOffset[1] = current.centerY;
